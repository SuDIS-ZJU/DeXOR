package algorithms.Elf.decoder;

import algorithms.Decoder;
import algorithms.Elf.Elf64Utils;
import enums.DataTypeEnums;
import utils.BinaryTools;

public class DoubleElfDecoder extends Decoder {
    ...
    public DoubleElfDecoder(String inputPath) {
        super(inputPath);
    }

    public DoubleElfDecoder(String inputPath, String config) {
    ...
    public double decodeDouble() {
        if (first) {
            ...
            int betaStar = 0;
            if (c1) betaStar = in.readInt(4);
            ElfXorDecoder();
            if (c1) return recover(previous_value, betaStar);
        }
