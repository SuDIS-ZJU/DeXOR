...
public class DoubleCamelEncoder extends Encoder {
    ...
    protected int maxDecimalPlaces = 4;

    public DoubleCamelEncoder(String outputPath) {
        super(outputPath);
    }

    public DoubleCamelEncoder(String outputPath, String config) {
        super(outputPath, config);
        try {
            maxDecimalPlaces = Math.max(0, Math.min(4,
                    Integer.parseInt(this.config.getOrDefault("decimal_places", "4"))));
    ...
    protected void decimal_compression(double value, double dec, int l) {
        out.write(l - 1, 2);
        double dxor = dec;
    ...
    protected void Camel(double value, long integer, int l) {
        double dec = value - integer;
        integer_encode(integer);
        decimal_compression(value, dec, l);
    ...
    public int encode(double value) {
        ...
        if (l > maxDecimalPlaces) {
            double scale = CamelTools.quick_pow10(maxDecimalPlaces);
