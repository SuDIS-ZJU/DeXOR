package algorithms.SElfStar.Huffman;
...
public class HuffmanEncode {
    ...
    public static void writeHuffmanCodes(StreamWriter out, Code[] huffmanCodes) {
        int maxLen = 0;
        ...
        out.write(bitsForLen, 3);    // 4 needs 4 bits
        for (Code huffmanCode : huffmanCodes) {
            out.write(huffmanCode.length - 1, bitsForLen);
            out.write(huffmanCode.code, huffmanCode.length);
        }
    }

    public static void readHuffmanCodes(StreamReader in, Code[] codes) {
        try {
