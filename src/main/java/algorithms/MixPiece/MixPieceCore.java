    ...
    private static void readByteArray(byte[] input) {
        ...
        if (decompressedSize <= 0 || decompressedSize > Integer.MAX_VALUE) {
            throw new IllegalArgumentException(
                    "Invalid or unsupported Zstd frame size: " + decompressedSize);
        }
        byte[] binary = Zstd.decompress(input, (int) decompressedSize);
