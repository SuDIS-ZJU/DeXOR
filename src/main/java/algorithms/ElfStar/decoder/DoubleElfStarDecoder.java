...
public class DoubleElfStarDecoder extends Decoder {
    ...
    public DoubleElfStarDecoder(String inputPath, String config) {
        super(inputPath, config);
        xorDecompressor = new ElfStarXORDecompressor(this.in);
    }

    public void decompress() {
        int encodedWindowSize = readInt(WINDOW_COUNT_BITS);
        currentWindowSize = encodedWindowSize == 0 ? window : encodedWindowSize;
        if (currentWindowSize < 1 || currentWindowSize > window) {
            throw new IllegalStateException("Invalid ElfStar window size: " + currentWindowSize);
        }
        initHuffmanTree();
        for (int i = 0; i < currentWindowSize; i++) {
            buffer[i] = nextValue();
        }
        available = currentWindowSize;
    }
    ...
    public double decodeDouble() {
        ...
        double res = buffer[currentWindowSize - available];
        available--;
        return res;
    }
}
