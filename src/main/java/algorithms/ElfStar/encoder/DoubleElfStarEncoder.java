...
public class DoubleElfStarEncoder extends Encoder {
    ...
    private static final int WINDOW_COUNT_BITS = 10;
    ...
    public DoubleElfStarEncoder (String outputPath, String config){
    ...
    private void compress() {
        if (numberOfValues <= 0 || numberOfValues > window) {
            throw new IllegalStateException("Invalid ElfStar window size: " + numberOfValues);
        }
        // Zero represents a full 1,000-value window. Other values encode the
        // exact final-window length so the decoder never consumes padding.
        out.write(numberOfValues == window ? 0 : numberOfValues, WINDOW_COUNT_BITS);
