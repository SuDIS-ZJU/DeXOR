package algorithms.Zstd.decoder;
...
public class DoubleZstdDecoder extends Decoder {
    private double[] currentBlock;
    private int blockPosition = 0;
    private int blockLength = 0;

    public DoubleZstdDecoder(String inputPath) {
        this(inputPath, "");
    }
    
    public DoubleZstdDecoder(String inputPath, String config) {
        super(inputPath);
    }
    ...
    private boolean readNextBlock() {
        try {
            ...
            byte[] uncompressed = Zstd.decompress(compressed, valueCount * 8);
