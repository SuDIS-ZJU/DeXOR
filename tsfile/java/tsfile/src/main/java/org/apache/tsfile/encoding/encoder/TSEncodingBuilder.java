...
public abstract class TSEncodingBuilder {
  ...
  public static class DeXOR extends TSEncodingBuilder {

    @Override
    public Encoder getEncoder(TSDataType type) {
      switch (type) {
        case DOUBLE:
          return new DoubleDeXOREncoder();
        default:
          throw new UnSupportedDataTypeException("DeXOR doesn't support data type: " + type);
      }
    }
  ...
  public static class EDeXOR extends TSEncodingBuilder {
    private int decimalPlaces = 2;

    @Override
    public Encoder getEncoder(TSDataType type) {
      if (type == TSDataType.DOUBLE) {
        return new DoubleEDeXOREncoder(decimalPlaces);
      }
      throw new UnSupportedDataTypeException("EDeXOR doesn't support data type: " + type);
    }

    @Override
    public void initFromProps(Map<String, String> props) {
      if (props != null && props.containsKey("decimal_places")) {
        decimalPlaces = Integer.parseInt(props.get("decimal_places"));
      }
      if (decimalPlaces < 0 || decimalPlaces > 15) {
        throw new IllegalArgumentException("decimal_places must be between 0 and 15");
      }
    }
  }
}
