...
public class DoubleDeXOREncoder extends GorillaEncoderV2 {
  ...
  protected void Decimal_XOR(double value, ByteArrayOutputStream out) {
    ...
    if (!Double.isFinite(value)
        || (absolute != 0 && (absolute < 1e-23 || absolute > Long.MAX_VALUE))) {
      writeBit(out);
      writeBit(out);
    ...
    if (cannotDecodeBitwise(value, alpha, beta, pow, delta)) { // Exception 10
      writeBit(out);
      writeBit(out);
      ExceptionHandle(value, out);
      return;
  ...
  private boolean cannotDecodeBitwise(
      double value, double alpha, long beta, double pow, int delta) {
    if (delta >= 16 || beta == Long.MIN_VALUE) {
      return true;
    }
    long magnitude = beta < 0 ? -beta : beta;
    if ((magnitude >> DeXORTools.decimalBits(delta)) != 0) {
      return true;
    }
    double decoded = alpha + beta * pow;
    return Double.doubleToRawLongBits(decoded) != Double.doubleToRawLongBits(value);
  }
}
